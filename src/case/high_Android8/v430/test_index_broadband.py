...
from src.case.high_Android8.bll.v430.index import close_broadband_alerts,close_index_alerts
...
class Test_BroadBand(TestCaseBase):
    ...
    def test_kuandai_service_enginor(self):
        ...
        try:
            # 查找智慧家庭--服务工程师xpath
            enginer_xpath = self.find_element("xpath", '//*[@content-desc="智慧家庭"]/android.widget.ListView[3]/android.view.View[4]/android.view.View[1]')
            if enginer_xpath is None:
                ...
                # 查看中服务工程师页面元素
                no_broadband_xpath = self.find_element("xpath", '//*[@content-desc="服务工程师"]/android.view.View[1]')
    ...
    def test_kuandai_check_bdpassword(self):
        ...
        try:
            ...
            # 查找宽带密码页面中提示框的的xpath
            no_broadband_xpath = self.find_element("xpath", '//*[@content-desc="宽带密码服务"]/android.view.View[1]')
            if no_broadband_xpath is None:
                self.driver.back()
