...
class TestCaseMy(TestCaseBase):
    ...
    def test_kyjf(self):
        """
         ...
         :return:
         """
        sore = self.find_element("id", "com.sunrise.scmbhc:id/id_mobile_score_txt")
        self.assertNotEqual(sore, None, "判断可用积分是否存在")
        sore = sore.text
        try:
            handle_sore = int(sore.replace("积分 ", ""))
        except:
            raise Exception
        logger.info("处理后积分为:{}".format(handle_sore))
        # 判断为空
        if handle_sore is None or handle_sore == "" or handle_sore == 0:
            self.assertEquals(1, 2, "可用积分余量为空,请检查我的-可用积分")

        if int(handle_sore) < 0:
            self.assertEquals(1, 2, "可用积分余量小于0,请检查我的-可用积分")

        # 判断可用积分类型，防止出现"--"的问题
        if not isinstance(handle_sore, int):
            self.assertEquals(1, 2, "可用积分格式异常,余量->{}".format(handle_sore))
    ...
    def test_user_num(self):
        ...
        self._click_my_button()
        # 获取手机号
        user_my_num = self.find_element("id", "com.sunrise.scmbhc:id/title_tv_1")
        self.assertNotEqual(user_my_num, None, "判断用户手机号存在")
        logger.info("获取到的手机号码是:{}".format(user_my_num.text))
    ...
    def test_user_star(self):
        ...
        """
        # 星级对象的父对象
        f_start_obj = self.find_element("id", 'com.sunrise.scmbhc:id/user_info_btn_explain')
        self.assertNotEqual(f_start_obj, None, "判断星级父元素存在")

        # 信用星级对象
        user_star = f_start_obj.text
