...
from src.case.high_Android8.bll.v430.index import close_index_alerts
from src.util.util_appium_tools import find_element
from src.case.high_Android8.bll.v430.index import _login_msg_alert
import time
...
def is_need_login(driver):
    ...
    try:
        ...
        login_but = find_element(driver, "id", "com.sunrise.scmbhc:id/id_my_mobile_login_btn")
        user_num_obj = find_element(driver, "id", "com.sunrise.scmbhc:id/id_mobile_city_1", timeout=1)
...
def one_key_login(driver):
    ...
    try:
        ...
        driver.find_element_by_id("com.sunrise.scmbhc:id/id_my_mobile_login_btn").click()

        is_selected = driver.find_element_by_id("com.sunrise.scmbhc:id/sms_mobile_deal").is_selected()
        if not is_selected:
            logger.info("登录复选框未勾选")
            driver.find_element_by_id("com.sunrise.scmbhc:id/sms_mobile_deal").click()


        find_element(driver=driver, by="id", value="com.sunrise.scmbhc:id/login_onekey_btn").click()
        time.sleep(5)
        _login_msg_alert(driver)
